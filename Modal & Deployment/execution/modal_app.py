...
async def fetch_meta_ads(data: dict, authorization: str = Header(...)):
    ...
    brand_info = brand_map[brand_key]
    ...
    # ── Helper: token expiry ──────────────────────────────────
    async def get_token_expiry(client):
    ...
    async def fetch_all():
        ...
        async with httpx.AsyncClient() as client:
            ...
            token_info = await get_token_expiry(client)
        return insights, adsets, ads, token_info

    # ── Execute ───────────────────────────────────────────────
    try:
        insights, adsets, ads, token_info = await asyncio.to_thread(asyncio.run, fetch_all())
    ...
    if days_left is not None:
        if days_left <= 0:
            ...
            token_warning = f"⚠️ Token Meta expired {days_left} hari lagi ({token_info.get('expires_on')}). Segera jalankan rotate_token.py"
    ...
    for entry in insights:
        ...
        # ATC & Purchase from catalog_segment_value
        atc_value = 0.0
        purchase_value = 0.0
        for v in (entry.get("catalog_segment_value") or []):
            if v.get("action_type") == "add_to_cart":
                atc_value = to_num(v.get("value"))
            elif v.get("action_type") == "purchase":
                purchase_value = to_num(v.get("value"))

        # ATC & Purchase qty from catalog_segment_actions
        atc_qty = 0.0
        purchase_qty = 0.0
        for v in (entry.get("catalog_segment_actions") or []):
            if v.get("action_type") == "add_to_cart":
                atc_qty = to_num(v.get("value"))
            elif v.get("action_type") == "purchase":
                purchase_qty = to_num(v.get("value"))

        spend = to_num(entry.get("spend"))
        ...
        rows.append({
            ...
            "ad_id": ad_id,
            "ad_name": entry.get("ad_name", "N/A"),
            "objective": resolve_objective(adset_meta),
            "spend": spend,
            "reach": to_num(entry.get("reach")),
            "frequency": to_num(entry.get("frequency")),
            "impressions": to_num(entry.get("impressions")),
            "link_click": to_num(entry.get("inline_link_clicks")),
            "cpm": to_num(entry.get("cpm")),
            "ctr": to_num(entry.get("inline_link_click_ctr")),
            "cpc": to_num(entry.get("cost_per_inline_link_click")),
            "atc_value": atc_value,
